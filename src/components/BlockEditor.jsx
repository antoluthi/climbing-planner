...
export function BlockEditor({ block, onUpdate, onRemove, canMoveUp, canMoveDown, onMoveUp, onMoveDown, allSessions, onCreateCustom, onSaveAsBlock, discipline = "climbing" }) {
  ...
  const inputStyle = {
    ...
    borderRadius: 4, color: colors(isDark).text,
    fontSize: 11, fontFamily: "inherit", padding: "3px 6px", outline: "none",
  };
  const labelStyle = { fontSize: 9, color: colors(isDark).textMuted, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 2 };
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 10px", cursor: "pointer" }} onClick={() => setOpen(o => !o)}>
        ...
        {(block.name || block.presetName) && (
          <span style={{ fontSize: 10, color: colors(isDark).textDim, fontStyle: "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, minWidth: 0 }}>
            {block.name || block.presetName}
          </span>
        )}
        ...
        <span style={{ fontSize: 10, color: colors(isDark).textMuted, flexShrink: 0 }}>{open ? "▲" : "▼"}</span>
        <div style={{ display: "flex", gap: 2, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
          <button style={{ background: "none", border: "none", cursor: canMoveUp ? "pointer" : "default", opacity: canMoveUp ? 0.7 : 0.2, fontSize: 11, color: colors(isDark).textDim, padding: "0 2px" }} onClick={onMoveUp} disabled={!canMoveUp}>↑</button>
          <button style={{ background: "none", border: "none", cursor: canMoveDown ? "pointer" : "default", opacity: canMoveDown ? 0.7 : 0.2, fontSize: 11, color: colors(isDark).textDim, padding: "0 2px" }} onClick={onMoveDown} disabled={!canMoveDown}>↓</button>
          <button style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, color: colors(isDark).danger, padding: "0 4px" }} onClick={onRemove}>✕</button>
        </div>
      </div>
      ...
      {open && (
        <div style={{ padding: "0 10px 10px", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {isClimbing && block.type === "Suspension" && suspCfg && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, background: colors(isDark).successBg, borderRadius: 6, padding: "10px 12px", border: `1px solid ${DATA.blocks["Suspension"]}44` }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: DATA.blocks["Suspension"], letterSpacing: "0.1em", textTransform: "uppercase" }}>Paramètres suspension</div>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    {[["two", "2 mains"], ["one", "1 main"]].map(([v, l]) => (
                      <button key={v} onClick={() => patchSusp({ armMode: v })}
                        style={{ flex: 1, padding: "4px 6px", borderRadius: 4, cursor: "pointer", fontSize: 10, fontFamily: "inherit", fontWeight: suspCfg.armMode === v ? 700 : 400, border: `1px solid ${suspCfg.armMode === v ? DATA.blocks["Suspension"] : (colors(isDark).border)}`, background: suspCfg.armMode === v ? colors(isDark).accentFaint : "none", color: suspCfg.armMode === v ? DATA.blocks["Suspension"] : muted }}>
                        {l}
                      </button>
                    ))}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    {[["wall", "Mur"], ["floor", "Sol"]].map(([v, l]) => (
                      <button key={v} onClick={() => patchSusp({ supportType: v })}
                        style={{ flex: 1, padding: "4px 6px", borderRadius: 4, cursor: "pointer", fontSize: 10, fontFamily: "inherit", fontWeight: suspCfg.supportType === v ? 700 : 400, border: `1px solid ${suspCfg.supportType === v ? DATA.blocks["Suspension"] : (colors(isDark).border)}`, background: suspCfg.supportType === v ? colors(isDark).accentFaint : "none", color: suspCfg.supportType === v ? DATA.blocks["Suspension"] : muted }}>
                        {l}
                      </button>
                    ))}
                  </div>
              ...
              <div>
                ...
                {suspCfg.armMode === "two" ? (
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    ...
                    {suspCfg.supportType === "wall" && suspCfg.targetWeight < 0 && <span style={{ fontSize: 9, color: colors(isDark).warn }}>délestage</span>}
                    {suspCfg.supportType === "wall" && suspCfg.targetWeight > 0 && <span style={{ fontSize: 9, color: DATA.blocks["Suspension"] }}>lest</span>}
                  </div>
                ) : (
                  <div style={{ display: "flex", gap: 8 }}>
                    <div style={{ flex: 1 }}>
          ...
          {isClimbing && block.type === "Suspension" && !suspCfg && (
            <button onClick={() => onUpdate({ config: { ...DEFAULT_SUSPENSION_CONFIG } })}
              style={{ padding: "6px 12px", background: colors(isDark).surface2, border: `1px solid ${colors(isDark).border}`, borderRadius: 5, color: DATA.blocks["Suspension"], cursor: "pointer", fontFamily: "inherit", fontSize: 10, fontWeight: 600 }}>
              ...
              {isClimbing ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <input type="range" min="0" max="10" step="1" style={{ ...styles.customFormSlider, accentColor: disciplineCfg.color }}
                    value={block.charge ?? 0} onChange={e => onUpdate({ charge: +e.target.value })} />
                  <span style={{ fontSize: 11, color: colors(isDark).textMuted, whiteSpace: "nowrap" }}>/ 10</span>
                </div>
              )}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
              ...
              {desc && (
                <button onClick={() => setDescPreview(p => !p)}
                  style={{ background: "none", border: `1px solid ${colors(isDark).border}`, borderRadius: 3, color: muted, padding: "1px 6px", cursor: "pointer", fontSize: 9, fontFamily: "inherit" }}>
          ...
          {onSaveAsBlock && (
            <button onClick={handleSaveAsBlock}
              style={{ alignSelf: "flex-start", padding: "4px 12px", background: "none", border: `1px solid ${accent}55`, borderRadius: 5, color: blockSaved ? (colors(isDark).success) : accent, cursor: "pointer", fontFamily: "inherit", fontSize: 10, fontWeight: 600 }}>
