    ...
    <Modal onClose={onComplete} maxWidth={460} dismissOnBackdrop={false} ariaLabel="Bienvenue dans Climbing Planner">
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <span style={{ fontSize: 10, fontWeight: 600, color: T.accent, letterSpacing: "0.1em", textTransform: "uppercase" }}>
            Bienvenue · {step + 1}/{SCREENS.length}
          </span>
          <button
            onClick={onComplete}
            aria-label="Passer"
            style={{ background: "none", border: "none", color: T.textLight, fontSize: 11, cursor: "pointer", fontFamily: "inherit", letterSpacing: "0.04em" }}
          >Passer →</button>
        </div>
        <div style={{ display: "flex", gap: 4 }} aria-label={`Étape ${step + 1} sur ${SCREENS.length}`}>
          {SCREENS.map((_, i) => (
            <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i <= step ? T.accent : T.border, transition: "background 0.2s" }} />
          ))}
        </div>
      </div>

      <ModalBody style={{ padding: "22px 22px 16px", gap: 0 }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 18 }}>
          <div aria-hidden="true" style={{
            width: 72, height: 72, borderRadius: "50%",
            background: screen.color + "22", border: `2px solid ${screen.color}55`,
            color: screen.color, fontSize: 36, fontWeight: 500,
        ...
        <div style={{ fontSize: 11, fontWeight: 600, color: T.textLight, letterSpacing: "0.1em", textTransform: "uppercase", textAlign: "center", marginBottom: 4 }}>
          {screen.eyebrow}
        </div>
        <div style={{ fontFamily: "'Newsreader', Georgia, serif", fontSize: 26, fontWeight: 500, color: T.text, textAlign: "center", lineHeight: 1.15, marginBottom: 14 }}>
          {screen.title}
        </div>
        <div style={{ fontSize: 14, color: T.textMid, lineHeight: 1.55, marginBottom: 16 }}>
          {screen.body}
        </div>
        <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 8 }}>
          {screen.bullets.map((b, i) => (
            <li key={i} style={{ fontSize: 13, color: T.textMid, lineHeight: 1.5, display: "flex", gap: 10, alignItems: "flex-start" }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: screen.color, flexShrink: 0, marginTop: 7 }} />
              <span>{b}</span>
            </li>
          ))}
        </ul>
      ...
      <ModalFooter align="between">
        <Button variant="ghost" size="md" onClick={() => setStep(s => Math.max(0, s - 1))} disabled={step === 0} style={{ visibility: step === 0 ? "hidden" : "visible" }}>
          ← Précédent
        </Button>
        {isLast ? (
          <Button variant="primary" size="md" onClick={onComplete}>Compris ✓</Button>
        ) : (
          <Button variant="primary" size="md" onClick={() => setStep(s => s + 1)}>Suivant →</Button>
        )}
      </ModalFooter>
    </Modal>
  );
