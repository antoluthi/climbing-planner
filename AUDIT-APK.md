...
> **Mise à jour (juillet 2026)** : l'ensemble des correctifs a été appliqué sur cette
> branche — voir les ✅ ci-dessous. Restent deux actions manuelles côté
> utilisateur : ajouter `com.climbingplanner.app://auth-callback` à l'allowlist
> Supabase (Auth > URL Configuration > Redirect URLs) pour le magic link APK,
> et configurer la signature release (4.1) quand une distribution est en vue.
> Point d'architecture : le bouton retour, la fermeture par Échap et le verrou
> de scroll passent désormais par une pile de calques partagée
> (`src/lib/native.js`), utilisée par `ui/Modal`, `DayLogModal` et `SessionModal`.
...
### 3.5 — ✅ (icônes) Barres système Android (statut / navigation)
